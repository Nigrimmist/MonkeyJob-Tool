    ...
    public class MtsEthernetBYModule : ModuleEventBase
    {
        ...
        public override void OnFire(Guid eventToken)
        {
            ...
            if (settings != null && settings.WarningBorders.Any())
            {
                ...
                {
                    _client.SendMessage(eventToken, CommunicationMessage.FromString("Заполните логин и пароль от сайта internet.mts.by"), messageType: MessageType.Error);
                    return;
                }
                ...
                if (topBorders.Any())
                {
                    ...
                    if (topBorder != null)
                    {
                        ...
                        if (shouldNotify)
                        {
                            ...
                            _client.SendMessage(eventToken, CommunicationMessage.FromString(string.Format("Время пополнить баланс интернета! На счету осталось {0} руб", amount)));
                ...
                {
                    settings.LastScannedAmount = amount;
                    _client.SendMessage(eventToken, CommunicationMessage.FromString(string.Format("Баланс интернета успешно пополнен до {0} руб", amount)));
                    _client.SaveSettings(settings);
                }
            }
        }
